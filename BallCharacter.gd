...
func _input(_event):
	if is_network_master():
		...
		if Input.is_action_just_pressed("Interact"):
			...
			var holding = hand.is_holding
			
			if holding and !target:
				game.rpc("spawn_object", hand.global_transform.origin, get_rotation(), holding)
				hand.remove(holding)
				return
			...
			if !holding and target:
				...
				if target.is_in_group("object"):
					target.interact()
					hand.hold(target.type)
					return
					
				if target.is_in_group("station"):
					if !target.current_item:
						return
					hand.hold(target.current_item)
					target.interact()
					return
					
			if holding and target:
				if target.is_in_group("station"):
					target.interact(holding)
					hand.remove(holding)
					return
